...
def build_all():
    ...
    # 生成供洛雪音乐导入的 JS 客户端代码内容
    client_script_content = f"""/**
 * @name MusicDL 自动化源
 * @description 实时解析音源
 * @version 1.0.0
 */
...
"""
    ...
    # 生成部署在 Cloudflare Worker 的全套后端服务（同时包含 API 和 JS 托管）
    worker_code = f"""const CLIENT_SCRIPT = `{client_script_content}`;

export default {{
  async fetch(request, env, ctx) {{
    const url = new URL(request.url);

    // 1. 如果访问 /lx-source.js，直接返回洛雪音乐客户端需要的自定义源文件
